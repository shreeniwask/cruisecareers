...
services.service('EditSchedulerActivity_Service',['$rootScope','$location','$http', function($rootScope, $location, $http) {
	...
	this.fetchActivityForIdByActivityId = function(id){
		var promise = $http({
			method : 'GET',
			url : 'rest/scheduler/fetchActivityForIdByActivityId/'+id,
			headers : {
				'Content-Type' : 'application/json'
			},
			cache : false
	...
	this.updateSchedulerData = function(user){
		var promise = $http({
			method : 'POST',
			data:user,
			url : 'rest/scheduler/updateSchedulerData',
			headers : {
				'Content-Type' : 'application/json'
			},
			cache : false
