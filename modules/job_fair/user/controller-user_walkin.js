...
controllers.controller('UserWalkin_Ctrl',['$scope','$rootScope','$state','GlobalModule_dataStoreService','GlobalModule_notificationService','Admin_Service','JobFairsList_Service','Userwalkin_Service', function ($scope, $rootScope,$state,GlobalModule_dataStoreService,GlobalModule_notificationService,Admin_Service,JobFairsList_Service,Userwalkin_Service)
{
    ...
    $scope.fetchStartInterviewDetails= function()
	{
		...
		//$scope.jobFairId=183;
		Userwalkin_Service.fetchStartInterviewDetails($scope.jobfairjobObj.id,$rootScope.userdetails.id).then(function(response){
	...
	$scope.getAppliedJobCountForUser=function(userId){
		  ...
		  Userwalkin_Service.getAppliedJobCountForUser(userid,$scope.jobfairjobObj.id).then(function(response){
...
$scope.selectedSlotCount=function(userId){
			...
			var userid;
			  ...
			  Userwalkin_Service.selectedSlotCount(userid,$scope.jobFairId).then(function(response){
	...
	$scope.fetchJobFairJobListForUser = function(search){
		  ...
		  if($rootScope.userdetails == undefined){
			  ...
			  userid=$rootScope.userdetails.id;}
		  var walkinflag=true;
		  Userwalkin_Service.fetchJobFairJobListForUser(search,userid,walkinflag).then(function(response){
	  ...
	  $scope.fetchJobFairImageListForUser = function(){
		  ...
		  Userwalkin_Service.fetchJobFairImageListForUser().then(function(response){
	  ...
	  $scope.fetchWalkinImageListForLandingPage = function(){
		  ...
		  Userwalkin_Service.fetchWalkinImageListForLandingPage().then(function(response){
	  ...
	  $scope.getJobFairJobListForUsercount=function(search){
			 ...
			 Userwalkin_Service.getJobFairJobListForUsercount($scope.search).then(function(response){
			...
			$scope.fetchJobById = function(jobid,walkinflag){		  
				  var userid;
				  if($rootScope.userdetails == undefined){
					  $state.go("user_job_fair_job_desc");
				  }else{
				  		$state.go("restricted.user_job_fair_job_desc");
				  		GlobalModule_dataStoreService.storeData('LoginModule','jobtypeflag',walkinflag);
				  }	
		   GlobalModule_dataStoreService.storeData('LoginModule','jobid',jobid);
			  ...
			  $scope.fetchDateAndTimeSlots = function(){
				  ...
				  Userwalkin_Service.fetchDateAndTimeSlots($scope.jobFairId,$scope.dateSelected).then(function(response){
					 ...
					 if($scope.listDate.length == 0)
					  {
						  GlobalModule_notificationService.notification("error","Slots for the selected date are currently unavailable.");
						  $('#time-select').hide();
						  $('.can').hide();
							 $('.canf').hide();
					  };
			...
			// $scope.fetchDateAndTimeSlots();
			  ...
			  $scope.saveSlotsOfUser=function(id){
				  ...
				  if($scope.slot.id != 0  || $rootScope.userdetails.STATUS=='inactive' || $rootScope.userdetails.STATUS=='Shortlisted' || $rootScope.userdetails.STATUS=='Selected'
					  ...
					  Userwalkin_Service.saveSlotsOfUser($scope.slot).then(function(response){
			  ...
			  $scope.fetchSelectedUserSlot = function(){
				  ...
				  Userwalkin_Service.fetchSelectedUserSlot($rootScope.userdetails.id,$scope.jobFairId).then(function(response){
...
// for applied job count
				...
				$scope.deleteUserSlot = function(){
					...
					Userwalkin_Service.deleteUserSlot($rootScope.userdetails.id).then(function(response){
						...
						//	$scope.hideArrowFunction();
							 ...
							 $scope.userid = $rootScope.userdetails.id;
							  $scope.fetchStatusOfCandidate=function(userid){
								  $scope.userid=userid;
								  Userwalkin_Service.fetchStatusOfCandidate($scope.userid).then(function(response){
