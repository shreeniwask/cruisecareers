...
controllers.controller('CreateJobFair_Ctrl',['$scope','$rootScope','$state','GlobalModule_dataStoreService','GlobalModule_notificationService','JobFairsList_Service','CreateJobFair_Service', function ($scope, $rootScope,$state,GlobalModule_dataStoreService,GlobalModule_notificationService,JobFairsList_Service,CreateJobFair_Service)
{
...
$scope.getRemoveForUSer = function(index){	
	...
	var validateJobFairDetail=function(jobfair)
	{
		...
		{
			GlobalModule_notificationService.notification("error","Please enter title for job fair");
			return false;
		}
		else if(!(jobfair.name.match(letterNumber)))
		{
			$(".loader").show();
			GlobalModule_notificationService.notification("error","Please enter valid title for job fair");
			return false;
		}
		if(jobfair.description == undefined || jobfair.description == '')
		{
			$(".loader").show();
			GlobalModule_notificationService.notification("error","Please enter description for job fair");
			return false;
		}
		else if(!(jobfair.description.match(letterNumber)))
		{
			$(".loader").show();
			GlobalModule_notificationService.notification("error","Please enter valid description for job fair");
			return false;
		}
		if(jobfair.activationDate == "")
		{
			$(".loader").show();
			GlobalModule_notificationService.notification("error","Please enter activation date for job fair");
			return false;
		}
		if(jobfair.startDate == "")
		{
			$(".loader").show();
			GlobalModule_notificationService.notification("error","Please enter start date for job fair");
			return false;
		}
		if(jobfair.endDate == "")
		{
			$(".loader").show();
			GlobalModule_notificationService.notification("error","Please enter end date for job fair");
			return false;
		}
	...
	$scope.disabled = 0; 
 	$scope.saveJobFairDetail=function(jobfair){
		...
		if(jobfair == undefined){
			...
			GlobalModule_notificationService.notification("error","Please enter mandatory fields for job fair");
			$(".loader").fadeOut("slow");
			return false;
		...
		jobfair.walkinflag= false;
		$(".loader").show();
		
		CreateJobFair_Service.saveJobFairDetail(jobfair).then(function(response){
			var savedStatus=response.data;
			
			if(savedStatus == 'success')
			{
				GlobalModule_notificationService.notification("success","Job Fair created successfully");
				$(".loader").fadeOut("slow");
			...
			*/
				  ...
				  GlobalModule_notificationService.notification("error","job fair is already active on this date");
				  $("#add_brand").modal('hide');
				  $(".loader").fadeOut("slow");				 
			 }
		...
		$(".loader").fadeOut("slow");
			...
			GlobalModule_notificationService.notification("error","Error In creating job fair");
			$(".loader").fadeOut("slow");
		});
	...
	$scope.updateJobFairDetail=function(jobfair){
		...
		CreateJobFair_Service.saveJobFairDetail(jobfair).then(function(response){
			...
			if(savedStatus == 'success')
			{
				GlobalModule_notificationService.notification("success","Job Fair updated successfully");
		...
		$scope.onloadEndDate=function(){
			...
			$scope.fetchUploadedJobfairImages=function(jobfair){
				...
				var walkinflag=false;
				  JobFairsList_Service.fetchUploadedJobfairImages($scope.jobfairId,walkinflag).then(function(response){
			  ...
			  $scope.fetchUploadedJobfairImagesForUser=function(jobfair){
				  ...
				  var walkinflag=false;
				  JobFairsList_Service.fetchUploadedJobfairImagesForUser($scope.jobfairId,walkinflag).then(function(response){
